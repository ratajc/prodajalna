  ...
  form.parse(zahteva, function(napaka1, polja, datoteke) {
    strankaIzRacuna(polja.seznamRacunov, function(napaka2, stranka) {
      pesmiIzRacuna(polja.seznamRacunov, function(napaka3, pesmi) {
        ...
        {
          console.log("napaka1 = " + napaka1 + ", napaka2 = " + napaka2 + ", napaka3 = " + napaka3);
          odgovor.sendStatus(500);
        }
        else
        {
...
streznik.get('/izpisiRacun/:oblika', function(zahteva, odgovor) {
  pb.all("SELECT * FROM Customer WHERE Customer.CustomerId = " + zahteva.session.CustomerId, function(napaka, vrstice) {
    if (napaka)
    {
      console.log(napaka);
      odgovor.sendStatus(500);
...
streznik.post('/prijava', function(zahteva, odgovor) {
  ...
  form.parse(zahteva, function (napaka1, polja, datoteke) {
    ...
    if (!napaka1)
    {
      // Poskus/ujem ne bo ujel asinhronizirano kodo. 
        var stmt = pb.prepare("\
    ...
    var s = "";
    if (napaka1 || napaka2)
      s = "Prišlo je do napake pri registraciji nove stranke. Prosim preverite vnešene podatke in poskusite znova";
    else
      s = "Stranka je bila uspešno registrirana";
      
    vrniStranke(function(napaka1, stranke) {
      vrniRacune(function(napaka2, racuni) {
        odgovor.render('prijava', {sporocilo: s, seznamStrank: stranke, seznamRacunov: racuni});
      });
    });
...
streznik.post('/odjava', function(zahteva, odgovor) {
    zahteva.session.CustomerId = null;
    zahteva.session.kosarica = [];
    odgovor.redirect('/prijava') 
})
